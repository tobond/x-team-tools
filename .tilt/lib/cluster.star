...
def validate_cluster_safety(cluster_type, debug_mode=False):
    """Validate cluster context for safety with comprehensive checks and enhanced error reporting"""
    ...
    try:
        ...
        fail("""
🚨 CLUSTER CONTEXT ERROR: Cannot determine current cluster context

Error: {}

TROUBLESHOOTING STEPS:
1. Verify kubectl is installed: kubectl version --client
2. Check kubeconfig exists: ls ~/.kube/config
3. List available contexts: kubectl config get-contexts
4. Set a valid context: kubectl config use-context <context-name>

COMMON FIXES:
• Docker Desktop: Enable Kubernetes in Docker Desktop settings
• Kind: Create cluster with: kind create cluster --name tilt-dev
• K3d: Create cluster with: k3d cluster create tilt-dev

Cannot proceed without valid cluster context.
        """.format(str(e)))
...
def detect_cluster_environment(current_context, debug_mode=False):
    ...
    is_local_api = any(indicator in api_server for indicator in local_api_indicators)
    
    # Validate context locality
    local_cluster_indicators = ['kind', 'k3d', 'docker-desktop', 'docker-for-desktop', 'minikube']
    is_local_context = any(indicator in current_context.lower() for indicator in local_cluster_indicators)
